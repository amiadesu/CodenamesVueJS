<template>
    <div :card_value="word.text" 
        class="word-block" 
        :class="[word.color, {glow : interfaceData.player.state.teamColor === word.color, revealed : word.revealed}]"
    ...
    >
        ...
        <p 
            class="word-text"
            :class="`size-${interfaceData.gameRules.fieldSize}`"
        >
...
export default defineComponent({
    ...
    inject: ['interfaceData', 'revealWord'],
    methods: {
        selectWord(event) {
            if (this.interfaceData.selecting === this.word.text) {
                clearInterval(this.selectionInterval);
                this.selectionProgress = 0;
                this.selectionIsGoing = false;
                ...
                this.interfaceData.selecting = wordText;
                this.selectionIsGoing = true;

                this.selectionInterval = setInterval(() => {
                    if (this.interfaceData.selecting !== wordText) {
                        clearInterval(this.selectionInterval);
                        this.selectionProgress = 0;
                        this.selectionIsGoing = false;
                        return;
                    ...
                    if (this.selectionProgress >= 1) {
                        ...
                        this.selectionIsGoing = false;
                        this.interfaceData.selecting = "";
                        this.revealWord(wordText);
                    }
                }, 10);
                // User selects a word.
            } else if (!this.interfaceData.player.state.master && 
                (this.interfaceData.gameProcess.masterTurn || this.interfaceData.player.state.teamColor !== this.interfaceData.gameProcess.currentTurn)) {
                this.counter++;
                if (this.counter > 100) {
                    this.counter = 0;
                }
