<template>
    <div v-if="player" class="player-wrapper">
        <div v-if="player.id === interfaceData.player.id" class="color-circle clickable" :style="{'background-color' : player.color}" @click="changeColor"></div>
        ...
        <span class="player-object-wrapper" 
            ...
            <span class="player-name-wrapper" :class="{clicker : player.clicker}">
                {{ player.name }}
            </span>
        </span>
        <div class="player-statuses-wrapper">
            <svg 
                ...
                viewBox="0 0 512 512"
                v-if="player.traitor && interfaceData.gameRules.gamemode === 'traitor'
                    && interfaceData.player.state.teamColor !== player.state.teamColor
                    && interfaceData.player.state.master"
            >
            ...
            <svg 
                ...
                viewBox="0 0 512 512"
                    @click="removePlayer"
                    @mouseenter="" 
                    v-if="player.id !== interfaceData.player.id && player.state.teamColor !== 'spectator'"
            >
            ...
            <svg 
                ...
                v-if="player.id !== interfaceData.player.id">
            >
...
export default defineComponent({
    ...
    methods: {
        changeColor() {
            this.interfaceData.player.color = '#' + (Math.random() * 0xFFFFFF << 0).toString(16).padStart(6, '0');
        },
        removePlayer() {
            alert(this.$t("codenames.rules.game_interface.alerts.remove_player"));
        },
        openHostTransferPanel() {
            alert(this.$t("codenames.rules.game_interface.alerts.promote_player"));
        },
        listenForUpdates() {
